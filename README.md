...
later once warm, so a cold start never blocks the reviewer. The deploy config
(`cloudbuild.yaml`, container Dockerfiles) lives on the `cloud-run` branch; the
full gcloud runbook is kept local-only (not in the repo).
