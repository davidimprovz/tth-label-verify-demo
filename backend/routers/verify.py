...
async def verify_refine(
    ...
    verdict. A NO-OP that returns the plain OCR verdict when the VLM tier is off,
    so the frontend can always call it without branching.
    """
    from backend import settings
    ...
    try:
        ...
        if not settings.vlm_enabled():
            return ocr_result  # tier off → plain OCR verdict (no-op)
